...
public class TicketService {
    ...
    public TicketEntity findById(UUID id) {
        return repository.findById(id)
                .orElseThrow(() -> new ApplicationException(ApplicationError.TICKET_NOT_FOUND, id));
    }
    ...
    public List<TicketResponse> createTickets(UUID eventId, TicketBatchRequest request) {
        var event = eventRepository.findById(eventId)
                .orElseThrow(() -> new ApplicationException(ApplicationError.EVENT_NOT_FOUND, eventId));
